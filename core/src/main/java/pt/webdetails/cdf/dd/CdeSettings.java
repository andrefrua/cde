    ...
    public List<PathOrigin> getComponentLocations() {
      ...
      for (Element pathElement : pathElements) {
        ...
        if(!StringUtils.isEmpty(origin)) {
          ...
          PluginSettings.logger.error("Must specify origin (static|repo), location '" + path + " 'ignored.");
        }
      }
      return locations;
    }
      ...
      if( xmlPathElements != null ) {
        ...
        for ( Element xmlPathElement : xmlPathElements ) {
          ...
          if ( StringUtils.isEmpty( path ) || StringUtils.isEmpty( origin ) ) {
            PluginSettings.logger.error( "Must specify origin (static|repo) and location '" + path + "." );
            continue;
          }
