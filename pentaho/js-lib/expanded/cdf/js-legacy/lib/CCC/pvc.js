...
pen.define("cdf-legacy/lib/CCC/pvc-d1.0", [ "cdf-legacy/lib/CCC/def", "cdf-legacy/lib/CCC/protovis", "cdf-legacy/lib/CCC/cdo", "cdf-legacy/jquery", "cdf-legacy/lib/CCC/tipsy" ], function(def, pv, cdo, $) {
    ...
    def("pvc.visual.SunburstDiscreteColorAxis", pvc.visual.ColorAxis.extend({
        methods: {
            ...
            _getBaseScheme: function() {
                var isFanOrLevelsMode = "slice" !== this.chart.plots.sunburst.option("ColorMode");
            ...
            _selectDomainItems: function(domainData) {
                ...
                return isFanOrLevelsMode ? def.query(domainData.childNodes).where(isNotDegenerate) : def.query(domainData.nodes()).where(function(itemData) {
        ...
        _buildScene: function() {
            ...
            function calculateColor(scene, index, siblingsSize, level) {
                ...
                if (parent) {
                    ...
                    if (!baseColor && !parent.isRoot() && (baseColor = parent.color)) if (isColorModeFan) index && colorBrightnessFactor && (baseColor = baseColor.brighter(colorBrightnessFactor * index / (siblingsSize - 1))); else if (isColorModeLevel) {
            ...
            var colorBrightnessFactor, sliceLevelAlphaRatio, sliceLevelAlphaMin, roles = this.visualRoles, rootScene = new pvc.visual.SunburstScene(null, {
                ...
                allowNestedVars: !0,
                hasPercentSubVar: !0
            }), colorGrouping = roles.color && roles.color.grouping, colorAxis = this.axes.color, colorMode = this.plot.option("ColorMode"), isColorModeFan = "fan" === colorMode, isColorModeLevel = "level" === colorMode, colorScale = roles.color.isBound() ? colorAxis.sceneScale({
                sceneVarName: "color"
            }) : def.fun.constant(colorAxis.option("Unbound")), levels = 0;
